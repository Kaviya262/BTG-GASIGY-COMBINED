...
const Addclaimpayment = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        modeOfPaymentId: Yup.string()
            ...
            .test("is-valid-mode", "Mode Of PaymentId is required", value => {
    ...
    useEffect(() => {
        ...
        loadClaimType(selectedCategory?.categoryid);
    ...
    const getClaimDetailsById = async (claimId) => {
        try {
            ...
            if (res?.status && res?.data) {
                ...
                const detailResult = mapItemsFromApi(res.data.details || [], header.ClaimCategoryId);
                ...
                setInitialValues({
                    ...
                    claimNumber: header.ApplicationNo ?? "",
                    ...
                    attachment: header.AttachmentName ?? null,
                ...
                setPreviewUrl(header.AttachmentPath ?? null)
                setFileName(header.AttachmentName ?? null)
    ...
    const handleSubmit = async (values, isSubmitted) => {
        ...
        // return;
        try {
            const res = await SaveClaimAndPayment(isEditMode, payload);
                                ...
                                <CardBody>
                                    <Formik initialValues={initialValues} validationSchema={validationSchema} enableReinitialize={true} innerRef={formikRef} context={{ isEditMode: true }}>
                                        {({ errors, touched, values, setFieldValue, setFieldTouched }) => {
                                            ...
                                            const handleCancel = () => { history.push('/Manageclaim&Payment') }
                                                            ...
                                                            <FieldArray name="items">
                                                                {({ push, remove }) => (
                                                                    <div style={{ overflowX: "auto" }}>
                                                                        ...
                                                                        <Table className="ClaimTable" style={{ minWidth: (selectedCategory?.categoryid === 3 && 1 === 0) ? "2800px" : "1600px" }} bordered>
                                                                            <thead style={{ backgroundColor: "#3e90e2", color: "#fff", textAlign: "center" }}>
                                                                                <tr>
                                                                                    <th width="2%"><button
                                                                                        type="button" disabled={isirn === 1}
...
export default Addclaimpayment;
