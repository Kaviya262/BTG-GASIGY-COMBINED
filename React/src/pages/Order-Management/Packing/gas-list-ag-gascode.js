...
const GasListAgGasCode = ({
  ...
  console.log("fromData", fromgasData);

  const [accordionState, setAccordionState] = useState(
    Object.entries(sofromData["GASList"] || {}).reduce((acc, [key, item]) => {
  ...
  const handleInputChange = (
    ...
    // Update the state
    setFromgasData(prevData => {
      if (
        !prevData[customerid] ||
        !prevData[customerid]["GASList"] ||
        !prevData[customerid]["GASList"][colKey]
      ) {
        return prevData;
      }
        ...
        [customerid]: {
          ...
          GASList: {
            ...
            [colKey]: {
              ...prevData[customerid]["GASList"][colKey],
              gasdetails: {
                ...prevData[customerid]["GASList"][colKey]["gasdetails"],
                [colKey2]: {
                  ...prevData[customerid]["GASList"][colKey]["gasdetails"][colKey2],
                  [field]: value,
                },
              },
            },
  ...
  return (
    <>
      <div className="accordion accordion-flush" id="accordionFlushExample">
        {fromgasData[customerid] &&
          fromgasData[customerid]["GASList"] &&
          Object.entries(fromgasData[customerid]["GASList"]).length > 0 ? (
          Object.entries(fromgasData[customerid]["GASList"]).map(
            ([colKey, item], index) => {
              const SO_label = item.label || `SO00${index + 1}`;
              const gasdetails = item.gasdetails || [];
                ...
                <div className="accordion-item" key={colKey}>
                  ...
                  <Collapse isOpen={isOpen} className="accordion-collapse">
                    <div className="accordion-body">
                      {gasdetails && Object.entries(gasdetails).length > 0 ? (
                        <div className="table-responsive mt-1">
                          <Table className="table mb-0">
                            <thead style={{ backgroundColor: "#3e90e2" }}>
                              <tr>
                                <th className="text-center" style={{ width: "20%" }}>Gas Name</th>
                                <th className="text-center" style={{ width: "11%" }}>SO No.</th>
                                <th className="text-center" style={{ width: "11%" }}>SQ No.</th>
                                <th className="text-center" style={{ width: "6%" }}>SO Qty</th>
                                <th className="text-center  required-label" style={{ width: "10%" }}>Pick Qty</th>
                                ...
                                ([colKey2, item2], index2) => {
                                  ...
                                  return (
                                    <tr key={`${colKey}-${colKey2}`}>
                                      ...
                                      <td>
                                        <Input type="text" value={item2.SO_Number} disabled />
                                      </td>
                                      <td>
                                      ...
                                      <td>
                                        <Select
                                          ...
                                          placeholder="Select"
                                          value={driverNameOptions.find(opt => opt.value === item2.drivername)}
                                          onChange={opt =>
                                            handleInputChange(
                                              "drivername",
                                              opt ? opt.value : "",
                        ...
                        <p className="text-center text-muted">
                          Please select customers and gas code.
                        </p>
                      )}
                    </div>
                  </Collapse>
...
export default GasListAgGasCode;
