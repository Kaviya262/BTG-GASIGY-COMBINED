...
const AddOrdernew = () => {
  ...
  const validationfn = (name, value, tempErrors) => {
    ...
    //   }
    // }
    console.log(name, value, tempErrors);
  ...
  const saveAddressdet = () => {
    if (addressrowids.length > 0) {
      ...
      setSODetail(prevState => {
        ...
        return [
          { ...prevState, ...deliveryaddressList[0] },
          ...prevState.slice(1),
        ];
      });
    }
  ...
  const handleQtyChange = (index, qty, col) => {
    if (!Array.isArray(gasCodeList[col])) return;

    const updatedDetails = [...gasCodeList[col]];
    updatedDetails[index] = {
      ...updatedDetails[index],
      SO_Qty: parseInt(qty, 10),
    };
    ...
    const updatedDetails1 = Array.isArray(SODetail) ? [...SODetail] : [];
    if (updatedDetails1[index]) {
      updatedDetails1[index] = {
        ...updatedDetails1[index],
        SO_Qty: parseInt(qty, 10),
      };
    } else {
      updatedDetails1[index] = { SO_Qty: parseInt(qty, 10) };
    }
    setSODetail(updatedDetails1);
  };
  const handlePoNumberChange = (index, val, col) => {
    if (!Array.isArray(gasCodeList[col])) return;
    const updatedDetails = [...gasCodeList[col]];
    updatedDetails[index] = { ...updatedDetails[index], PoNumber: val };
    ...
    const updatedDetails1 = Array.isArray(SODetail) ? [...SODetail] : [];
    if (updatedDetails1[index]) {
      updatedDetails1[index] = { ...updatedDetails1[index], PoNumber: val };
    } else {
      updatedDetails1[index] = { PoNumber: val };
    }
    setSODetail(updatedDetails1);
  };

  const handleUOMChange = (index, uom, col) => {
    if (!Array.isArray(gasCodeList[col])) return;
    ...
    setGasCodeList(updatedDetails);

    const updatedDetails1 = Array.isArray(SODetail) ? [...SODetail] : [];
    updatedDetails1[index] = { ...updatedDetails1[index], uomid: uom };
    setSODetail(prevState => ({
      ...prevState,
      updatedDetails1,
    }));
  ...
  useEffect(() => {
    setErrorMsg([]);
    if (!gasCodeList || Object.keys(gasCodeList).length === 0) return;

    Object.entries(gasCodeList).forEach(([colKey, mergedGasCodes]) => {
      fnsetsqdetails(colKey, mergedGasCodes);
    });
  }, [gasCodeList]);

  const fnsetsqdetails = (colKey, mergedGasCodes) => {
    setSQDetails(prevState => {
      const existingSqids = new Set(prevState.map(item => item.Sqid));
      const newEntries = mergedGasCodes
        .filter(val => !existingSqids.has(val.SQ_ID))
        .map(val => ({
          SO_ID: 0,
          Sqid: val.SQ_ID,
        }));
  ...
  useEffect(() => {
    console.log("SQDetails", SQDetails);
    console.log("GasCodeList", gasCodeList);
  }, [gasCodeList, SQDetails]);
  ...
  const handleSubmit = async (e, actionType) => {
    ...
    // ✅ Extract selected gas code rows dynamically
    const SODetail = Object.values(gascoderows)
        .flat()
        .filter(item => item.chkStatus === true || item.chkStatus === 1);

    // Validation for selected gas codes
    if (SODetail.length === 0) {
        tempErrors["gasCode"] = "Please select at least one gas code.";
    }

    // Validate gas code detail fields
    SODetail.forEach((item, index) => {
        if (!item.PoNumber || item.PoNumber.trim() === "") {
            tempErrorsArr.push(`Gas Detail Row ${index + 1}: Po No. is required.`);
        }
        if (!item.SO_Qty || isNaN(parseFloat(item.SO_Qty)) || parseFloat(item.SO_Qty) < 1) {
            tempErrorsArr.push(`Gas Detail Row ${index + 1}: Please enter a valid SO Qty.`);
        }
    });

    // Combine all validation errors
    tempErrorsArr = [...tempErrorsArr, ...Object.values(tempErrors)];
    if (tempErrorsArr.length > 0) {
        setErrorMsg(tempErrorsArr);
        return;
    }
  ...
  useEffect(() => {
    setSoData(prevState => ({ ...prevState, ["Categories"]: soBaseValue }));
  }, [soBaseValue]);
  ...
  const openaddresspopup = async (rowItem, colKey, index) => {
    toggleModal();
    try {
      const rowdetail = { ...gasCodeList[colKey][index], colKey, index };
      const data = await GetShippingAddress(rowItem.ContactId, -1);
      SetDelAddress(data);
      setDeliveryaddressList([rowdetail]);
    } catch (error) {
      console.error("Error fetching customer data:", error);
    }
  };
